...
class PracticeRound:
    ...
    def _advance(self):
        ...
        if self.current:
            ...
            self.message = self.question.get('reason') or '请回忆答案；Enter 检查。'
        else:
            self.question = {}
            self.message = f'本轮结束：已答 {self.answered}，独立答对 {self.correct}，待加强 {self.errors}。'
