...
class TerminalStudy:
    def __init__(self, session, store, audio_settings=None, pronouncer=None, library=None, planner=None, routine=None):
    ...
    def open_menu(self):
        ...
        self.picker_rows = [
            ...
            {'value': 'automatic', 'label': '2. 自动学习 / 返回学习'},
    ...
    def maybe_start_incremental(self):
        if not self.planner or not self.routine.config['extra_after_daily'] or not self.daily_work_complete():
            return False
    ...
    def advance_daily_work(self):
        ...
        if not self.routine.config['extra_after_daily']:
            return False
    ...
    def draw_daily(self, screen):
        ...
        self.put(screen, rows - 1, 0, 'Esc 菜单  F9 复习  ^Q 保存退出', curses.A_DIM)
        screen.refresh()
    ...
    def handle_picker(self, key):
        ...
        try:
            if self.picker == 'menu':
                ...
                if value in ('extra_after_daily', 'examples_during_learning'):
...
def main(argv=None):
    ...
    try:
        with StateStore(args.data_dir, import_from=old_progress) as store:
            ...
            if args.configure:
                print(json.dumps(routine.config, ensure_ascii=False, indent=2))
                planner.close()
                return 0
            if args.stats:
                stats = planner.stats()
                ...
                planner.close()
                return 0
            app = TerminalStudy(session, store, library=library, planner=planner, routine=routine)
