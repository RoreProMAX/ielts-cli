...
CONTEXTS = {"learn", "review", "repair", "extra"}
...
class StudyPlanner:
    ...
    def due_cards(self, dictionary_id=None, limit=200, now=None, exclude_reviewed_today=False):
        ...
        if exclude_reviewed_today:
            ...
            query += " AND NOT EXISTS (SELECT 1 FROM events WHERE events.dictionary_id=cards.dictionary_id AND events.word=cards.word AND context='review' AND happened_at>=? AND happened_at<?)"
        ...
        try:
            ...
            if context == "repair":
                ...
                if correct and not hinted:
                    # 增量练习不改变正式复习节奏；错误或提示仍进入短期重测。
                    due, new_stage = row[0] if was_existing else moment + 600, stage
                else:
                    due, new_stage = moment + 600, 0
    ...
    def has_seen(self, dictionary_id, word):
        self._check_key(dictionary_id, 'dictionary_id')
        self._check_key(word, 'word')
        return self.db.execute('SELECT 1 FROM cards WHERE dictionary_id=? AND word=?', (dictionary_id, word)).fetchone() is not None
    ...
    def stats(self, day=None):
        ...
        extra_words = {(dictionary_id, word) for dictionary_id, word, mode, is_correct, context in rows if context == 'extra'}
        extra_attempts = sum(1 for row in rows if row[4] == 'extra')
        return {"new_words": len(seen_learn), "review_words": len(seen_review), "extra_words": len(extra_words), "extra_attempts": extra_attempts, "attempts": len(rows), "correct": correct, "due": due, "by_mode": by_mode}
