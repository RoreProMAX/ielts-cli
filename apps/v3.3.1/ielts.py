...
class TerminalStudy:
    ...
    def update_rows(self):
        ...
        rows = [
            ('auto_check', '启动时检查更新：' + ('开' if self.updater.auto_check else '关')),
            ('channel', '更新通道：' + ('稳定版（切换为 beta）' if self.updater.channel == 'stable' else 'beta（切换为稳定版）')),
            ('check_update', '立即检查更新'),
        ]
    ...
    def poll_updates(self):
        ...
        if state == 'available':
            self.update_banner = ('beta v' if '-beta.' in status['version'] else '新版 v') + status['version'] + ' 可用；Esc 菜单 10 可选择更新。'
    ...
    def handle_update_choice(self, value):
        if value == 'auto_check':
            ...
            self.updater.set_channel('beta' if self.updater.channel == 'stable' else 'stable')
            self.update_dismissed_version = None
            self.open_update_picker()
    ...
    def open_routine_picker(self):
        ...
        if self.updater:
            self.picker_rows.append({'value': 'auto_update_check', 'label': '9. 启动时检查更新：' + ('开' if self.updater.auto_check else '关')})
    ...
    def draw(self, screen):
        ...
        if self.help_open:
            lines = [
                ...
                'Esc菜单第10项：stable/beta通道、更新开关与确认安装',
                'Ctrl+Q / Ctrl+C 保存退出',
...
def main(argv=None):
    ...
    parser.add_argument('--auto-update-check', choices=('on', 'off'), help='启动时后台检查所选通道；可配合 --configure 保存')
    parser.add_argument('--update-channel', choices=('stable', 'beta'), help='更新通道，默认 stable；beta 需主动开启')
    ...
    try:
        ...
        if args.update_status:
            status = dict(bootstrap_updater.poll(), current_version=VERSION, auto_check=bootstrap_updater.auto_check, channel=bootstrap_updater.channel,
    ...
    try:
        with StateStore(args.data_dir, import_from=old_progress) as store:
            ...
            if args.update_channel is not None:
                update_manager.set_channel(args.update_channel)
            ...
            if args.configure:
                print(json.dumps(dict(routine.config, auto_update_check=update_manager.auto_check, update_channel=update_manager.channel), ensure_ascii=False, indent=2))
