...
class UpdateManager:
    """Manage a single profile's opt-in, staged stable updates."""

    def __init__(self, data_dir, current_version, *, transport=None):
        parsed = _parse_version(current_version)
        if parsed is None:
            raise ValueError("current_version must be a plain X.Y.Z version")
        ...
        self._ready = None
        self._status = {"state": "idle", "version": current_version, "message": "尚未检查更新"}
        ...
        self.auto_check = self._load_auto_check()

    def _load_auto_check(self):
        try:
            value = json.loads(self.settings_path.read_text(encoding="utf-8"))
        except (OSError, UnicodeError, json.JSONDecodeError):
            return True
        enabled = value.get("auto_check") if isinstance(value, dict) else None
        return enabled if isinstance(enabled, bool) else True

    def set_auto_check(self, enabled):
        if not isinstance(enabled, bool):
            raise TypeError("enabled must be bool")
        self._atomic_json(self.settings_path, {"schema": 1, "auto_check": enabled})
    ...
    def _start(self, state, target):
        with self._lock:
            ...
            message = "正在下载并校验更新" if state == "downloading" else "正在检查稳定版"
            self._status = {"state": state, "version": version, "message": message}
            self._thread = threading.Thread(target=self._run_task, args=(target,), daemon=True,
    ...
    def _check(self):
        ...
        for attempt in range(2):
            ...
            try:
                response = self._request(LATEST_RELEASE_URL, {"api.github.com"},
        ...
        release = _read_json_bytes(payload, "GitHub 发布信息")
        if self._closed.is_set():
            return
        if release.get("draft") is not False or release.get("prerelease") is not False:
            self._candidate = None
            self._set_status("current", self.current_version, "当前已是最新稳定版")
            return
        tag = release.get("tag_name")
        version = tag[1:] if isinstance(tag, str) and tag.startswith("v") else None
        parsed = _parse_version(version)
        if parsed is None or tag != "v" + version or parsed <= self._current_tuple:
            self._candidate = None
            self._set_status("current", self.current_version, "当前已是最新稳定版")
            return
        ...
        if self._candidate_is_staged(candidate):
            ...
            self._set_status("ready", version, "稳定版 %s 已校验并暂存，等待你确认切换" % version)
            return
    ...
    def activate_ready(self):
        ...
        with self._lock:
            self._ready = None
        self._set_status("ready", ready["version"], "稳定版 %s 已设为下次启动版本" % ready["version"])
        return launcher
