...
from quiz import QuestionFactory, is_correct, choose_mode
...
class TerminalStudy:
    def __init__(self, session, store, audio_settings=None, pronouncer=None, library=None, planner=None):
        ...
        self.practice_settings = {'version': 1, 'learn_mode': 'copy', 'review_mode': 'recall'}
        if self.practice_settings_path.exists():
            value = json.loads(self.practice_settings_path.read_text(encoding='utf-8'))
            if not isinstance(value, dict) or type(value.get('version')) is not int or value['version'] != 1 or value.get('learn_mode') not in MODE_LABELS or value.get('review_mode') not in MODE_LABELS:
    ...
    def persist(self):
        if not self.dirty:
            return
        if self.round:
            path = self.round_path if self.round.context == 'review' else self.learn_round_path
    ...
    def make_question(self, identifier, word, mode):
        ...
        if mode == 'mixed':
            mode = choose_mode(self.planner.mode_stats(identifier, word), has_content=word.casefold() in factory.content) if self.planner.has_seen(identifier, word) else 'copy'
        if mode == 'copy':
    ...
    def load_round(self, context='review'):
        ...
        path = self.round_path if context == 'review' else self.learn_round_path
        if not path.exists():
            return None
        if path.stat().st_size > 2 * 1024 * 1024:
            raise ValueError('复习轮次文件过大')
        return PracticeRound.restore(self.library.catalog, self.planner, json.loads(path.read_text(encoding='utf-8')), provider=self.make_question, checker=is_correct)

    def start_quiz(self, mode=None, limit=20, resume=True):
        ...
        if pending and pending.mode == mode and pending.current['dictionary_id'] == self.library.active_id:
            self.round = pending
        else:
            ...
            self.round = PracticeRound(self.library.catalog, self.planner, cards, mode=mode, context='learn', provider=self.make_question, checker=is_correct)
    ...
    def pause_review(self):
        if self.round:
            ...
            self.pronouncer.cancel()
            self.cursor = len(self.session.state['draft'])
            self.notice = '本轮已暂停，输入已保留；F9 进入复习，F10 选择学习题型。'
    ...
    def draw_round(self, screen):
        ...
        activity = '复习' if round_.context == 'review' else '学习'
        label = MODE_LABELS.get(round_.question.get('mode'), '默写')
        self.put(screen, 0, 0, f'雅思随手练 v{VERSION} {activity} {label} {book} 已答{round_.answered} 待答{round_.remaining}', curses.A_BOLD)
        ...
        if round_.phase != 'answer':
            ...
            self.put(screen, 1, 0, round_.question['prompt'])
            options = round_.question['options']
            for index, option in enumerate(options):
                if rows >= 8:
                    self.put(screen, index + 2, 0, f'{index + 1}. {option}')
                else:
                    cell = (cols - 2) // 2
                    self.put(screen, 2 + index // 2, (index % 2) * (cell + 1), clipped(f'{index + 1}. {option}', cell))
        else:
        ...
        self.put(screen, rows - 1, 0, '1–4选择' if choice and cols < 65 else ('1–4选择 F3答案 F4跳过 F5读音 F10题型 ^Q退出' if choice else 'Enter检查 F3答案 F4跳过 F5读音 F9复习 F10题型 ^Q退出'), curses.A_DIM)
        try:
    ...
    def draw_picker(self, screen):
        rows, cols = screen.getmaxyx()
        title = '词库选择' if self.picker == 'dictionary' else ('题型选择 / ' + ('复习' if self.picker_scope == 'review_mode' else '学习') if self.picker == 'mode' else '章节选择 / ' + self.library.entry['name'])
