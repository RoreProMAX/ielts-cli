...
_POS_PREFIX_RE = re.compile(r"^(?:n|v|vt|vi|adj|adv|prep|conj|pron|num|aux)\.?\s*", re.I)
...
class QuestionFactory:
    ...
    def _fallback(self, word, requested, prompt=None, reason=None, mode="recall"):
        ...
        return {
            "mode": mode,
            "prompt": prompt or (meanings[0] if meanings else "请跟打当前单词。"),
            "options": [],
            "correct_indices": [],
            "answer": word["name"],
            "word": word["name"],
    ...
    def _choice(self, word, mode):
        ...
        if len(candidates) < 3:
            prompt = self._meanings(word)[0] if self._meanings(word) else None
        ...
        return {
            "mode": mode,
            "prompt": word["name"] if mode == "en_to_zh" else self._meanings(word)[0],
            "options": options,
    ...
    def make(self, word_name, mode):
        ...
        if not meanings:
            return self._fallback(word, mode, reason="该词没有中文释义，改为跟打练习，不计入默写熟练度。", mode="copy")
        return {"mode": "recall", "prompt": meanings[0], "options": [], "correct_indices": [], "answer": word["name"], "word": word["name"]}
...
def choose_mode(stats, has_content=False):
    if not _familiar(stats, "en_to_zh"):
        return "en_to_zh"
    if not _familiar(stats, "zh_to_en"):
        return "zh_to_en"
    if not _familiar(stats, "recall"):
        return "recall"
    ...
    return min(candidates, key=attempts)
