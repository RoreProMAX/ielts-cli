...
def main():
    ...
    if args.child_result:
        return child(args.child_result, args.utf8_console)
    ...
    with tempfile.TemporaryDirectory(prefix='ielts-width-probe-') as temporary:
        for utf8 in (False, True):
            destination = Path(temporary) / ('utf8.json' if utf8 else 'default.json')
            command = [sys.executable, '-X', 'utf8', '-B', str(Path(__file__).resolve()),
                       '--child-result', str(destination)]
            if utf8:
            ...
            try:
                while time.monotonic() - started < 15 and session.is_alive():
                    ...
                    if chunk:
                        events.append({'mode': 'utf8' if utf8 else 'default', 'output': chunk})
                if session.is_alive():
                ...
                results.append({'mode': 'utf8' if utf8 else 'default', 'probe_error': str(error)})
            finally:
                session.close()
    completed = all('measurements' in result for result in results)
    (output / 'width-probe.json').write_text(json.dumps({'diagnostic_completed': completed, 'variants': results}, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
