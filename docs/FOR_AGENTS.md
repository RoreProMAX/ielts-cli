...
5. 升级时先复制旧版本进度到新版本目录，保留源目录。不要让旧版和新版同时写同一份数据。

后台提醒默认关闭。只有用户明确要求时才执行 `--reminders enable`；这会创建本程序的 Linux 用户定时器。它不是普通启动检查的一部分。
...
| 版本管理 | 入口、目录和打包工具中有版本映射 | 集中版本元数据，旧包与数据兼容测试，避免只改 VERSION |
...
```sh
python3 tools/package_release.py dist/IELTS-CLI-Portable-3.2.0.zip
```
