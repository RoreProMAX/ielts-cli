...
设置参数包括 `--configure`、`--new-goal N`、`--review-goal N`、`--batch-size N`、`--reminder-minutes N`、`--extra-after-daily on|off`、`--examples-during-learning on|off`。提醒管理使用 `--reminders enable|disable|status`；其退出后后台实现仅支持 Linux。
