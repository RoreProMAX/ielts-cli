...
| 参数 | 作用 |
|---|---|
| `--app-version 1\|2\|3` | 选择应用版本，默认 `3`（V3.3.0） |
...
设置参数包括 `--configure --new-goal N`、`--review-goal N`、`--batch-size N`、`--reminder-minutes N`、`--extra-after-daily on|off`、`--examples-during-learning on|off`、`--auto-update-check on|off`。`--update-status` 只读显示更新状态；`--no-update` 仅本次跳过检查；提醒管理使用 `--reminders enable|disable|status`，其退出后后台实现仅支持 Linux。
